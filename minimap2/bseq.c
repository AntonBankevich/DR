...
mm_bseq_file_t *mm_bseq_open(const char *fn, runtime_sequences *s)
{
	mm_bseq_file_t *fp;
	gzFile f;
...
//	    printf("%d %d\n", ks->seq.l, ks->seq.m);
//        printf("%s\n%s\n%d %lu %d %lu\n%s\n%s\n%d %lu %d %lu\n", ks->name.s, ks->s->names[ks->s->cur], ks->name.l, strlen(ks->s->names[ks->s->cur]),
//        ks->name.m, ((strlen(ks->s->names[ks->s->cur]) + 31) & ~31u),
//        ks->seq.s, ks->s->s[ks->s->cur],
//        ks->seq.l, strlen(ks->s->s[ks->s->cur]),
//        ks->seq.m, ((strlen(ks->s->s[ks->s->cur]) + 31) & ~31u));
//        ks->s->cur += 1;
...
{
	return (ks_eof(fp->ks) && fp->s.seq == 0);
}
