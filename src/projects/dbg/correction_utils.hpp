...
inline dbg::Edge * checkBorder(dbg::Vertex &v) {
...
inline bool checkInner(dbg::Vertex &v) {
    size_t inc_rel = 0;
    size_t out_rel = 0;
    for(dbg::Edge &edge : v.rc()) {
        if(edge.is_reliable)
...
inline void FillReliableWithConnections(logging::Logger &logger, dbg::SparseDBG &sdbg, double threshold) {
...
inline std::unordered_map<dbg::Vertex *, size_t> findReachable(dbg::Vertex &start, double min_cov, size_t max_dist) {
...
inline std::vector<dbg::GraphAlignment> FindPlausibleBulgeAlternatives(const dbg::GraphAlignment &path,
