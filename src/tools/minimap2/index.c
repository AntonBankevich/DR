...
mm_idx_t *mm_idx_build(const char *fn, runtime_sequences *s, int w, int k, int flag, int n_threads) // a simpler interface; deprecated
{
	mm_bseq_file_t *fp;
	mm_idx_t *mi;
	fp = mm_bseq_open(fn, s);
	if (fp == 0) return 0;
...
mm_idx_reader_t *mm_idx_reader_open(const char *fn, runtime_sequences *s, const mm_idxopt_t *opt, const char *fn_out)
{
    int64_t is_idx;
    mm_idx_reader_t *r;
    ...
    if (r->is_idx) {
        ...
        r->idx_size = is_idx;
    } else r->fp.seq = mm_bseq_open(fn, s);
